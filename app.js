...
const dbPath = path.join(__dirname, "moneyMaintain.db");
const app = express();
app.use(cors());
...
app.post("/postData/", authenticateToken, async (req, res) => {
  try {
    const { category, type, amount = 0, transactionDate } = req.body;
...
app.delete("/delete/:id", authenticateToken, async (req, res) => {
  const { id } = req.params;
  const isIdCheck = `select * from userTransactions where id =${id}`;
  const isIdExist = await db.get(isIdCheck);
  if (isIdExist === undefined) {
    res.status(401);
    res.send({ message: "transaction is already deleted" });
  } else {
    const sqlQery = `delete  from userTransactions where id=${id}`;
    const dbRes = await db.run(sqlQery);
...
app.put("/update/:id", authenticateToken, async (req, res) => {
  ...
  const sqlQuery = `update userTransactions set category='${category}',
  type='${type}',
  amount=${amount},
